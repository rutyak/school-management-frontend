...
const Navbar = ({
  ...
  return (
    <nav
      className="flex-1 bg-white xl:rounded-xl shadow-md border p-3 xl:px-6 xl:py-4 flex items-center justify-between gap-3 z-20 
      md:h-18 lg:h-20 transition-all duration-300"
    >
      <div className="flex items-center gap-4">
        <GiHamburgerMenu
          className="text-2xl md:text-3xl text-gray-700 cursor-pointer hover:text-blue-500 xl:hidden"
          onClick={handleToggleMenu}
        />
        <h1 className="text-[18px] md:text-2xl font-bold text-gray-800">
          {linkType.toUpperCase()}
        </h1>
      </div>

      {!hideInput && (
        <div className="relative w-[60%] xl:w-[50%] flex justify-center">
